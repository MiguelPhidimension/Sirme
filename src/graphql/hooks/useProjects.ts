...
const CREATE_CLIENT_MUTATION = `
  mutation CreateClient($name: String!) {
    insert_clients_one(object: {name: $name}) {
      client_id
      name
    }
  }
 ...
 *
 * @returns Function to create a client
 *
 * @example
 * ```tsx
 * const createClient = useCreateClient();
 *
 * const newClient = await createClient({ name: 'New Client' });
 * ```
 */
export const useCreateClient = () => {
  return $(async (clientData: { name: string }) => {
    try {
      const variables = {
